...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node [fuzzyBlock, label=below:$PD$, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, right = of derivada] (controller) {};

    \node [block1, minimum height=2em, minimum width=1em, above = of controller] (integrador) {$\frac{1}{s}$};
    \node [Gain, right = of integrador] (gananciaI) {\tiny{$K_i$}};
    ...
    \draw [->] (empty2) |- (integrador);
    \draw [-] (controller) -- (empty3);
    \draw [->] (integrador) -- (gananciaI);
    \draw [->] (gananciaI) -| node[pos=0.97, left] {\tiny{$+$}} (sum2);
    \draw [->] (empty3) -- (sum2);
    \draw [->] (sum2) -- (actuador);
