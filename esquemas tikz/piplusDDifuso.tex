...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node [fuzzyBlock, label=below:$PI$, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, right = of derivadajoint] (controller) {};
    \node [Gain, above = of controller] (gananciaD) {\tiny{$K_d$}};

    \node [block1, minimum height=2em, minimum width=1em, right = of controller] (integrador) {$\frac{1}{s}$};
    \node [sum, right = of integrador] (sum2) {};
    ...
    \node [block1, node distance=1.2cm, below = of integrador] (sensor){$H(s)$};
    ...
    \draw [->] (derivadajoint) |- (gananciaD);
    \draw [->] (controller) -- (integrador);
    \draw [->] (gananciaD) -| node[pos=0.97, left] {\tiny{$+$}} (sum2);
    \draw [->] (integrador) -- (sum2);
    \draw [->] (sum2) -- (actuador);
