...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node [block1, right = of errorjoint] (controllerpid) {$PID$};
    \node [fuzzyBlock, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, above = of controllerpid] (controller) {};
    
    \node [sum, right = of controllerpid] (sum2) {};
    ...
    \node [block1, node distance=1.2cm, below = of actuador] (sensor){$H(s)$};
    

    \draw [draw,->] (input) node[above right] {$sp$} -- (sum);
    \draw [-] (sum) -- (errorjoint);
    \draw [->] (errorjoint) -- (controllerpid);
    \draw [->] (errorjoint) |- (controller);
    \draw [->] (controller) -| node[pos=0.97, left] {\tiny{$+$}} (sum2);
    \draw [->] (controllerpid) -- (sum2);
    \draw [->] (sum2) -- (actuador);
