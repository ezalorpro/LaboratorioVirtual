...
\begin{tikzpicture}[auto, >=latex']
    ...
    \node[branch, node distance=3.5cm, right = of empty1] (empty2) {};
    
    \node [branch, node distance=0.4cm, right = of derivada] (derivadajoint) {};
    \node [fuzzyBlock, label=below:$PD$, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, right = of derivadajoint] (controller) {};
    \node [fuzzyBlock, label=above:$PI$, minimum height=2em, minimum width=3em, node distance=0.6cm, draw, above = of controller] (controller2) {};

    \coordinate[node distance=0.8cm, right = of controller] (empty3) {};
    \node [block1, minimum height=2em, minimum width=1em, right = of controller2] (integrador) {$\frac{1}{s}$};
    \node [sum, right = of empty3] (sum2) {};
    \node [block1, right = of sum2] (actuador) {$A(s)$};
    ...
    \node [block1, node distance=1.2cm, below = of empty3] (sensor){$H(s)$};
    ...
    \draw [->] (empty2) |- ([yshift=1em]controller2);
    \draw [->] (derivada) |- (controller);
    \draw [->] (derivadajoint) |- (controller2);
    \draw [-] (controller) -- (empty3);
    \draw [->] (controller2) -- (integrador);
    \draw [->] (integrador) -| node[pos=0.97, left] {\tiny{$+$}} (sum2);
    \draw [-] (empty3) -- (sum2);
    \draw [->] (sum2) -- (actuador);
