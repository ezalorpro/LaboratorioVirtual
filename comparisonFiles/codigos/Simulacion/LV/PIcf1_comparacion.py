...
MatFileMATLAB = io.loadmat('comparisonFiles/Data MATLAB/Simulacion/PIcf8', squeeze_me=True)
MatFileSciLab = io.loadmat('comparisonFiles/Data SciLab/Simulacion/PIcf8', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Simulacion/Controlador10.pkl', 'rb') as f:
    t_lv, yout_lv, yc_lv, set_point, _ = pickle.load(f)

with open('comparisonFiles/Data LVSCCD/Simulacion/Controlador11.pkl', 'rb') as f:
...
ax.plot(t_comun, yout_lvf, 'orange', dashes=[1, 2], alpha=0.4, label='LV/RK2 sin Filtro', linewidth=3)
ax.plot(t_comun, yout_sci, color="#12711C", dashes=[2, 2], label='SciLab/LSODAR', linewidth=2)
ax.set_title('Controlador PI difuso')
ax.legend(loc=1)
ax.grid()

axins = ax.inset_axes([0.55, 0.09, 0.4, 0.33])
...
axins.grid()
axins.set_xlim(5 - 0.5, 5 + 0.3)
axins.set_ylim(1.42, 1.51)

ax.indicate_inset_zoom(axins)
fig.tight_layout()
plt.savefig('comparisonFiles/plots/Simulacion/PIcf1.pdf')
plt.show()