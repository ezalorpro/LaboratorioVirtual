...
MatFile = io.loadmat('comparisonFiles/Data SciLab/Analisis/S2Nichols', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Analisis/Set2Nichols.pkl', 'rb') as f:
...
ax.plot(Pha1, Db2, color="#12711C", label='SciLab', linewidth=2)
...
ax.fill_between(Pha1, Db1, Db2, alpha=0.4, color="#12711C", label='Area de diferencia')
ax.set_title('Diagrama de Nichols para el Sistema 2')
...
nichols_grid(figure=fig, ax=ax, offsetP=10)

axins = ax.inset_axes([0.12, 0.17, 0.32, 0.33])
axins.plot(Pha1, Db2, color="#12711C", label='SciLab', linewidth=2)
...
axins.fill_between(Pha1, Db1, Db2, alpha=0.4, color="#12711C", label='Area de diferencia', interpolate=True)
axins.xaxis.set_major_locator(plt.MaxNLocator(2))
# axins.yaxis.major.formatter.set_powerlimits((0, 0))

x1, x2 = Pha1[indice] - np.abs(Db1[indice] - Db2[indice])*5, Pha2[indice] + np.abs(Db1[indice] - Db2[indice])*5
...
plt.savefig('comparisonFiles/plots/Analisis/SciLab/ScSet2Nichols.png', dpi=200)
plt.show()
