...
MatFile = io.loadmat('comparisonFiles/Data MATLAB/Analisis/S3Rlocus', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Analisis/Set3Rlocus.pkl', 'rb') as f:
...
print(NRe2[index_max])

print(f'{"Valor real de diferencia maxima: ":<38}{tempRe2[indice]:.4f}')

fig, ax = plt.subplots(figsize=(5.1, 4.2))

for index, _ in enumerate(NImg2):
    if index == 0:
        ax.plot(NRe2[index], NImg2[index], color="#001C7F", label='MATLAB', linewidth=2)
    else:
        ax.plot(NRe2[index], NImg2[index], color="#001C7F", linewidth=2)


for index, _ in enumerate(NImg1):
    if index == 0:
        ax.plot(NRe1[index], NImg1[index], 'r', dashes=[1, 2], label='Laboratorio Virtual', linewidth=3)
    else:
        ax.plot(NRe1[index], NImg1[index], 'r', dashes=[1, 2], linewidth=3)
...
print(f'{"Distancia de energia: ":<38}{energy_distance(tempImg1, tempImg2):.3E}')
print(f'{"Raiz del Error cuadratico medio: ":<38}{np.sqrt((np.subtract(tempImg1, tempImg2)**2).mean()):.3E}')
