...
MatFile = io.loadmat('comparisonFiles/Data SciLab/Analisis/S1Step', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Analisis/Set1Step.pkl', 'rb') as f:
    T1, Y1 = pickle.load(f)
...
ax.plot(T2, Y2, color="#12711C", label='SciLab', linewidth=2)
...
ax.fill_between(T2, Y1, Y2, alpha=0.4, color="#12711C", label='Area de diferencia')
ax.set_xlabel('tiempo')
ax.set_title('Respuesta escalón para el Sistema 1')
ax.legend(loc=7, bbox_to_anchor=(0.97, 0.65))
ax.grid()

axins = ax.inset_axes([0.55, 0.13, 0.4, 0.3])
axins.plot(T2, Y2, color="#12711C", label='SciLab', linewidth=2)
...
axins.fill_between(T2, Y1, Y2, alpha=0.4, color="#12711C", label='Area de diferencia')
...
fig.tight_layout()
plt.savefig('comparisonFiles/plots/Analisis/SciLab/ScSet1Step.png', dpi=200)
plt.show()
