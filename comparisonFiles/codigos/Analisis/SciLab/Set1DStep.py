...
MatFile = io.loadmat('comparisonFiles/Data SciLab/Analisis/S1DStep', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Analisis/Set1DStep.pkl', 'rb') as f:
    T1, Y1 = pickle.load(f)
...
ax.step(T2, Y2, where='mid', color="#12711C", label='SciLab', linewidth=2)
...
axins.step(T2, Y2, where='mid', color="#12711C", label='SciLab', linewidth=2)
...
axins.fill_between(T2, Y1, Y2, step="mid", alpha=0.4, color="#12711C", label='Area de diferencia')
...
fig.tight_layout()
plt.savefig('comparisonFiles/plots/Analisis/SciLab/ScSet1DStep.png', dpi=200)
plt.show()
