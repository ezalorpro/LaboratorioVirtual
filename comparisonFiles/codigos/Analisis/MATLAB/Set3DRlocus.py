...
MatFile = io.loadmat('comparisonFiles/Data MATLAB/Analisis/S3DRlocus', squeeze_me=True)

with open('comparisonFiles/Data LVSCCD/Analisis/Set3DRlocus.pkl', 'rb') as f:
...
ax.set_xlim(-5, 5)
ax.set_ylim(-5, 5)
ax.set_title('Lugar de las raíces para el sistema 3')
ax.legend()
ax.grid()

axins = ax.inset_axes([0.15, 0.08, 0.35, 0.3])
for index, _ in enumerate(Img2):
    if index == 0:
        axins.plot(Re2[index], Img2[index], color="#001C7F", label='MATLAB', linewidth=2)
    else:
        axins.plot(Re2[index], Img2[index], color="#001C7F", linewidth=2)

for index, _ in enumerate(Img1):
    if index == 0:
        axins.plot(Re1[index],
                   Img1[index],
                   'r',
                   dashes=[1, 2],
                   label='Laboratorio Virtual',
        ...
        axins.plot(Re1[index], Img1[index], 'r', dashes=[1, 2], linewidth=3)
axins.xaxis.set_major_locator(plt.MaxNLocator(2))
# axins.yaxis.major.formatter.set_powerlimits((0, 0))

x1, x2 = tempRe2[indice] - np.abs(tempImg1[indice] - tempImg2[indice]), tempRe2[indice] + np.abs(tempImg1[indice] - tempImg2[indice])

if tempImg2[indice] >= tempImg1[indice]:
    y1, y2 = tempImg1[indice] - np.abs(tempImg1[indice] - tempImg2[indice]), tempImg2[indice] + np.abs(tempImg1[indice] - tempImg2[indice])
else:
    y1, y2 = tempImg2[indice] - np.abs(tempImg1[indice] - tempImg2[indice]), tempImg1[indice] + np.abs(tempImg1[indice] - tempImg2[indice])

axins.set_xlim(-0.01, 0.01)
...
print(f'{"Distancia de energia: ":<38}{energy_distance(np.hstack(Img1), np.hstack(Img2)):.3E}')
print(
